...
void data_report_timeout_handler(void *p_context)
{
    ...
    err_code = sd_nvic_SetPriority(TEMP_IRQn, NRF_APP_PRIORITY_LOW);
    APP_ERROR_CHECK(err_code);
...
static void TEMP_IRQ_handler(void *p_event_data, uint16_t event_size)
{	
	int32_t     temp_result;
...
void TEMP_IRQHandler(void)
{
	...
	if (NRF_TEMP->EVENTS_DATARDY != 0)
	{
		...
		// Schedule ADC event
		err_code = app_sched_event_put(&temp_result, sizeof(int32_t), TEMP_IRQ_handler);
		APP_ERROR_CHECK(err_code);
	}
}
